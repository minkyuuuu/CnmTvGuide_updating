...
public class Remote_My_CH extends ListActivity {

	public static final String DEBUG_TAG = "Remote_My_CH";
	...
	public class TVCH_All_Adapter extends BaseAdapter{
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				convertView = mInflater.inflate(R.layout.remocon_channel_row, null);
				//Log.d("Sangtae", "convertView Null");
				holder = new ViewHolder();
				//holder.text = (TextView) convertView.findViewById(R.id.TextView01);
				// 아이콘
				holder.Mych_Icon	= (ImageView)convertView.findViewById(R.id.icon_mych);
				holder.Ch_num		= (TextView)convertView.findViewById(R.id.ch_number);
				holder.log_img		= (ImageView) convertView.findViewById(R.id.channel_logo);

				holder.Ch_Subject	= (TextView)convertView.findViewById(R.id.program_subject);
			...
			if (tItem.getCHANNEL_onAir_TIME() != null) {
				holder.Ch_Subject.setText(ToHourTime(tItem.getCHANNEL_onAir_TIME())+tItem.getCHANNEL_onAIR_TITLE());
			}
			// 로그
			String TempIMG = tItem.getCHANNEL_LOG_IMG();
			if (TempIMG != null) {
				imageDownloader.download(tItem.getCHANNEL_LOG_IMG(), holder.log_img, null);//, BitmapFactory.decodeResource(getResources(), R.drawable.noimg_logo));
			}
			//My ch
	...
	class TvAllAsyncTask extends AsyncTask<Void, Void, TV_ALL_DATA_LIST>
	{
		...
		protected TV_ALL_DATA_LIST doInBackground(Void... params) {
			...
			if(tempApp.getTv_ALL_DATA_LIST().size() == 0 || tempApp.getTVCH_SettigChecked())
			{
				...
				//TV_ALL_PASER tv_ALL_PASER = new TV_ALL_PASER("3", "11", "all");
