...
public class TVCH_Paid extends ListActivity{

	public static final String DEBUG_TAG = "TVCH_Paid";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		tempApp.setTv_ALL_DATA_LIST(mList);					// TV_ALL 채널 데이터을 Application에 저장함.
		 */
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		//parentActivity.startChildActivity("TVCH_Schedule", mList.get(position).getNAME());
	...
	class TvAllAsyncTask extends AsyncTask<Void, Void, TV_ALL_DATA_LIST>
	{
		...
		protected void onPostExecute(TV_ALL_DATA_LIST result) {
			...
			try
			{
				...
				if (tempApp.getTv_PAY_DATA_LIST().size() == 0) {
					...
					for (int i = 0; i <= temp; i++) {
						if (mSortDataMap.containsKey(String.valueOf(i))) {
							if (!TextUtils.isEmpty(mSortDataMap.get(String.valueOf(i)).getINFO())) {
								if (mSortDataMap.get(String.valueOf(i)).getINFO().equals("Pay") || mSortDataMap.get(String.valueOf(i)).getINFO().equals("HP")) {
									mList.add(mSortDataMap.get(String.valueOf(i)));
								}
							}
						}
					}
					tempApp.setTv_PAY_DATA_LIST(mList);
				} else {
					mList.clear();
					mList.addAll(tempApp.getTv_PAY_DATA_LIST());
				}
