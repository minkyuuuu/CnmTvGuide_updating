...
public class WorkspaceView extends ViewGroup {
  ...
  public boolean onInterceptTouchEvent(MotionEvent ev) {
	  Log.d("Sangtae", "onInterceptTouchEvent");
    if (locked) {
      return true;
    }
    ...
    Log.d("Sangtae", "action: "+action+"x: "+x+"y: "+y);
    switch (action) {
      case MotionEvent.ACTION_MOVE:
        ...
        Log.d("Sangtae", "touchSlop: "+touchSlop+"xMoved: "+xMoved+"yMoved: "+yMoved);
        if (xMoved || yMoved) {

          if (xMoved && !yMoved) {
            // Scroll if the user moved far enough along the X axis
        	  Log.d("Sangtae", "onInterceptTouchEvent 터치 모드를 스크롤 모드로 변경");
    ...
    switch (action) {
      ...
      case MotionEvent.ACTION_MOVE:
        if (touchState == TOUCH_STATE_SCROLLING) {
          ...
          lastMotionX = x;
          Log.d("Sangtae", "onTouchEvent" + "터치 다운시 좌우 스크롤 X 좌표: "+ deltaX);
          if (deltaX < 0) {
            if (getScrollX() > 0) {
      ...
      case MotionEvent.ACTION_UP:
        if (touchState == TOUCH_STATE_SCROLLING) {
          ...
          Log.d("Sangtae", "onTouchEvent" + "velocityX: "+ velocityX);
  ...
  public void scrollToScreen(int whichScreen) {
    ...
    Log.d("Sangtae", "newX=" + newX + " scrollX=" + getScrollX() + " delta=" + delta +"스크롤 이동값: "+Math.abs(delta) * 2);
