...
public class RecordList_PASER {
	...
	// deth 2
	private final static String XML_RECORD_ITEM	=	"Record_Item";
	// deth 3
	...
	private final static String XML_RECORD_DURATION		=	"RecordDuration";

	private static RECORD_DATA_LIST mRecord_LIST;

	public RecordList_PASER(String aURL) {
		...
		try {
			...
			while (eventType != XmlPullParser.END_DOCUMENT) {
				if(eventType == XmlPullParser.START_DOCUMENT) {
					...
					} else if (xpp.getName().equals(XML_RECORD_ITEM)) {
						//eventType = xpp.next();
